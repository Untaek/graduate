...
char buf[200];
int datalen = sizeof(struct Data);
...
void loop() {
  ...
  if(Serial.available()) {
    Serial.readBytes(buf, DATALEN);
    memcpy_P(&sensorData, buf, DATALEN);

    Serial.println("from mega");
    Serial.println(sensorData.measured_value);
